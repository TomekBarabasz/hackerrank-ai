...
TEST(TestRing, test_01)
{
    ...
    ASSERT_TRUE(ring.push_back(1));
    ASSERT_FALSE(ring.empty());
    ASSERT_TRUE(ring.push_back(2));
    ASSERT_TRUE(ring.push_back(3));
    ASSERT_FALSE(ring.push_back(4));
...
TEST(TestRing, test_02)
{
    ...
    ring.push_back(3);
    ASSERT_EQ(1,ring.popfirst());
    ASSERT_TRUE(ring.push_back(4));
    ASSERT_EQ(2,ring.popfirst());
    ASSERT_TRUE(ring.push_back(5));
    ASSERT_EQ(3,ring.popfirst());
    ASSERT_TRUE(ring.push_back(6));
...
TEST(Partition,test_01)
{
    ...
    for (int idx=0;idx<ss.nrow*ss.ncol;++idx){
        ASSERT_EQ(exp[idx], ss.groupAt(idx));
    }
    delete pss;
}
TEST(Partition,test_011)
{
    auto * pss = makeGrid({"112","132","122"});
    auto & ss = *pss;
    auto wrk = Workspace(ss.nrow,ss.ncol);
    int ng = partitionGrid(ss,wrk);
    ASSERT_EQ(3,ng);
    const int8_t exp[] = {1, 1, 2, 1, 3, 2, 1, 2, 2};
    for (int idx=0;idx<ss.nrow*ss.ncol;++idx){
...
TEST(Partition,test_03)
{
    ...
    const int8_t exp[] = {1, 2, 3, 3, 2, 2, 4, 4, 5, 2, 6, 4, 7, 2, 2, 4, 8, 2, 2, 9};
    for (int idx=0;idx<ss.nrow*ss.ncol;++idx){
        ASSERT_EQ(exp[idx], ss.groupAt(idx));
    }
    delete pss;
...
TEST(CollectGroups,test_01)
{
    ...
    int ng = partitionGrid(ss,wrk);
    ASSERT_EQ(3,ng);
    collectGroups(ss,ng,wrk,ss.blocks);
    ...
    //group 1
    {
        ...
        ASSERT_EQ(0,first);
        ASSERT_EQ(4,nelem);
        ASSERT_EQ( Point(0,0),blocks.points[0]);
        ASSERT_EQ( Point(0,1),blocks.points[1]);
        ASSERT_EQ( Point(1,0),blocks.points[2]);
        ASSERT_EQ( Point(2,0),blocks.points[3]);
    }
    //group 2
    {
        ...
        ASSERT_EQ(4,first);
        ASSERT_EQ(4,nelem);
        ASSERT_EQ( Point(0,2),blocks.points[4]);
        ASSERT_EQ( Point(1,2),blocks.points[5]);
        ASSERT_EQ( Point(2,1),blocks.points[6]);
        ASSERT_EQ( Point(2,2),blocks.points[7]);
    }
    //group 3
    {
        ...
        ASSERT_EQ(8,first);
        ASSERT_EQ(1,nelem);
        ASSERT_EQ( Point(1,1),blocks.points[8]);
    }
    delete pss;
}
TEST(updateColumn,test_01)
{
    ...
    for (int idx=0;idx<pss->nrow*pss->ncol;++idx)
        ASSERT_EQ(exp[idx], pss->gridAt(idx));
    delete pss;
}
...
TEST(removeGroup,test_01)
{
    ...
    auto *nss = removeGroup(*pss,0,wrk);    //groups are sorted by size!
    auto * grid = nss->grid;
    ...
    for (int idx=0;idx<nss->nrow*nss->ncol;++idx)
        ASSERT_EQ(exp[idx], nss->gridAt(idx));
    delete pss;
    delete nss;
}
TEST(solve,test_01)
